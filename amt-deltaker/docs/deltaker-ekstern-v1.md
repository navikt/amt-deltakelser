...
Deltakere kan bli slettet, da vil det bli produsert en tombstone for den deltakeren. En tombstone er en melding med
`null` som verdi, hvor key er deltakerens id.

Topicen er satt opp med evig retention og compaction, så den skal inneholde alle deltakere som har vært registrert på de
nevnte tiltakene. Compaction betyr at kun siste melding for hver key beholdes. Konsumenter som leser fra starten av
topicen vil få siste versjon av hver deltaker.
...
| Felt             | Format         | Beskrivelse               |
|------------------|----------------|---------------------------|
| **enhetsnummer** | `string\|null` | Enhetsnummer til veileder |
| **navIdent**     | `string`       | NAV-ident til veileder    |
...
- [DeltakerEksternV1Dto](https://github.com/navikt/amt-deltaker/blob/main/src/main/kotlin/no/nav/amt/deltaker/deltaker/kafka/dto/DeltakerEksternV1Dto.kt) (
  Skjema for deltakere)
