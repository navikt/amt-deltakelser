...
object DeltakerProgresjonHandler {
    ...
    fun getAvsluttendeStatusUtfall(deltakere: List<Deltaker>): List<Deltaker> {
        ...
        val fremtidigAvsluttendeStatusList =
            DeltakerStatusRepository.getAvsluttendeDeltakerStatuserForOppdatering(
                deltakere.map { it.id }.toSet(),
            )

        val deltakereMedFremtidigeAvsluttendeStatus =
            deltakere
                .mapNotNull { deltaker ->
    ...
    fun tilDeltar(deltakere: List<Deltaker>): List<Deltaker> =
        deltakere
    ...
    private fun getSluttarsak(deltaker: Deltaker): DeltakerStatus.Aarsak? =
        if (deltaker.deltakerliste.erAvlystEllerAvbrutt()) {
            DeltakerStatus.Aarsak(
                type = DeltakerStatus.Aarsak.Type.SAMARBEIDET_MED_ARRANGOREN_ER_AVBRUTT,
                beskrivelse = null,
            )
