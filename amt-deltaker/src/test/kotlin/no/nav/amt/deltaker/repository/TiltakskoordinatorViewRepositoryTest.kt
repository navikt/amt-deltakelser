...
class TiltakskoordinatorViewRepositoryTest {
    private val repository = TiltakskoordinatorViewRepository()
    ...
    fun `skal returnere tom liste når ingen deltakere finnes for gjennomføring`() {
        val result = repository.getDeltakere(UUID.randomUUID())
    ...
    fun `skal ikke returnere deltakere fra andre gjennomføringer`() {
        ...
        val result = repository.getDeltakere(deltakerliste1.id)
    ...
    inner class FlereDeltakelserTests {
        ...
        fun `skal returnere flere rader når samme person har flere deltakelser`() {
            val deltakerliste = lagDeltakerliste()
            val navBruker = no.nav.amt.lib.testing.utils.TestData
                .lagNavBruker()
            ...
            result shouldHaveSize 2
            result.map { it.personident }.toSet() shouldHaveSize 1
        }

        @Test
        fun `skal ha korrekte berikede felt per deltaker når flere finnes`() {
            val deltakerliste = lagDeltakerliste()
            val navBruker = no.nav.amt.lib.testing.utils.TestData
                .lagNavBruker()
            val ansatt = lagNavAnsatt()
            ...
            // Forslag kun på deltaker2
            forslagRepository.upsert(lagForslag(deltakerId = deltaker2.id, status = Forslag.Status.VenterPaSvar))

            // Vurdering kun på deltaker1
            vurderingRepository.upsert(
                lagVurdering(deltakerId = deltaker1.id, vurderingstype = Vurderingstype.OPPFYLLER_KRAVENE),
            )
            ...
            val row1 = result.single { it.id == deltaker1.id }
            val row2 = result.single { it.id == deltaker2.id }

            row1.harAktivtForslag shouldBe false
            row1.sisteVurderingstype shouldBe Vurderingstype.OPPFYLLER_KRAVENE

            row2.harAktivtForslag shouldBe true
            row2.sisteVurderingstype.shouldBeNull()
        }
    }
}
