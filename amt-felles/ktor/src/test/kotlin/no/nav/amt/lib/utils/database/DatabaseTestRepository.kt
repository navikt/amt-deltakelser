...
class DatabaseTestRepository {
    ...
    suspend fun cleanUp() = Database.query { session ->
        session.update(queryOf("TRUNCATE TABLE foo"))
    }

    suspend fun insert(n: Int) = Database.query { session ->
        session.update(queryOf("INSERT INTO foo (id) VALUES (?)", n))
    }

    suspend fun get(n: Int) = Database.query { session ->
        val queryAction =
            queryOf("SELECT * FROM foo WHERE id = ?", n)
                .map { r -> r.int("id") }
                .asSingle
