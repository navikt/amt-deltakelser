...
class KoordinatorServiceTest(
    ...
    @Nested
    inner class KometMasterTests {
        @Test
        fun `har tilgang, lagt til deltakerliste, komet er master - returnerer deltakerliste med deltakere uten endringsmld`() {
        ...
        @Test
        fun `har tilgang, lagt til deltakerliste - returnerer deltakerliste med deltakere inkl veiledere og aktiv endring`() {
            every { unleashToggle.erKometMasterForTiltakstype(any<Tiltakskode>()) } returns true
            val personIdent = "12345678910"
            val overordnetArrangorId = UUID.randomUUID()
            arrangorRepository.insertOrUpdateArrangor(
        ...
        @Test
        fun `har tilgang, lagt til deltakerliste, veileder - returnerer deltakerliste med deltakere, erVeilederForDeltaker`() {
            val personIdent = "12345678910"
            val overordnetArrangorId = UUID.randomUUID()
            arrangorRepository.insertOrUpdateArrangor(
                ArrangorDbo(
        ...
        @Test
        fun `har tilgang, deltaker har bade innsoktdato og forsteVedtakFattet, setter sokt inn til forsteVedtakFattet`() {
            val personIdent = "12345678910"
            val deltakerlisteId = UUID.randomUUID()
            val overordnetArrangorId = UUID.randomUUID()
            arrangorRepository.insertOrUpdateArrangor(
