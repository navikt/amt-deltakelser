...
import no.nav.amt.lib.utils.polymorphicCollectionToPGObject
...
class DeltakerRepository(
    ...
    fun insertOrUpdateDeltaker(deltakerDbo: DeltakerDbo) {
        ...
        template.update(
            ...
            sqlParameters(
                ...
                "vurderinger" to deltakerDbo.vurderingerFraArrangor?.let { objectMapper.polymorphicCollectionToPGObject(it) },
                "adressebeskyttet" to deltakerDbo.adressebeskyttet,
                "innhold" to deltakerDbo.innhold?.let { objectMapper.toPGObject(it) },
                "kilde" to deltakerDbo.kilde?.name,
                "historikk" to objectMapper.polymorphicCollectionToPGObject(deltakerDbo.historikk),
    ...
    fun oppdaterVurderingerForDeltaker(
        deltakerId: UUID,
        oppdaterteVurderinger: List<Vurdering>,
    ) {
        val sql =
            """
        ...
        template.update(
            sql,
            sqlParameters(
                "vurderinger" to objectMapper.polymorphicCollectionToPGObject(oppdaterteVurderinger),
                "deltakerId" to deltakerId,
            ),
        )
    }

    fun oppdaterEnhetsnavnForDeltakere(
        opprinneligEnhetsnavn: String,
        nyttEnhetsnavn: String,
    ) {
        val sql =
            """
            update deltaker
