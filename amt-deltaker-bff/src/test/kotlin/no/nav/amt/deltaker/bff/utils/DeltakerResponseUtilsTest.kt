...
import no.nav.amt.deltaker.bff.navtiltakskoordinator.auth.TiltakskoordinatorTilgangContext
import org.junit.jupiter.api.Test

class DeltakerResponseUtilsTest {
    @Test
    fun `visningsnavn - adressebeskyttet og ikke tilgang - sensurerer navn`(): Unit = with(
        TiltakskoordinatorTilgangContext(),
    ) {
    ...
    @Test
    fun `visningsnavn - adressebeskyttet og tilgang - sensurerer ikke navn`(): Unit = with(
        TiltakskoordinatorTilgangContext(),
    ) {
        medFortroligDeltaker()
    ...
    @Test
    fun `visningsnavn - ikke adressebeskyttet og tilgang - sensurerer ikke navn`(): Unit = with(
        TiltakskoordinatorTilgangContext(),
    ) {
        val (fornavn, mellomnavn, etternavn) = deltaker.navBruker.getVisningsnavn(true)
