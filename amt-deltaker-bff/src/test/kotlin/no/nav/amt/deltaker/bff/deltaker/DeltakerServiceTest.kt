...
class DeltakerServiceTest {
    ...
    inner class OppdaterDeltakerTests {
        @Test
        fun `deltaker er endret - oppdaterer`() =
            runTest {
        ...
        @Test
        fun `deltaker endringshistorikk mangler men er utkast - oppdaterer`() =
            runTest {
                val deltaker =
                    lagDeltaker(
        ...
        @Test
        fun `deltakerstatus er endret - oppdaterer`() =
            runTest {
        ...
        @Test
        fun `deltaker kan ikke endres - oppdaterer deltaker men beholder lasing`() =
            runTest {
                val sistEndret = LocalDateTime.now().minusDays(3)
        ...
        @Test
        fun `deltaker kan ikke endres, kun oppdatert historikk - oppdaterer historikk`() =
            runTest {
                val sistEndret = LocalDateTime.now().minusDays(3)
                val deltaker = lagDeltaker(sistEndret = sistEndret, kanEndres = false)
                TestRepository.insert(deltaker)
        ...
        @Test
        fun `oppdaterDeltaker(endring) - kaller client og returnerer deltaker`() =
            runTest {
                ...
                endringRequests.forEach { endringRequest ->
                    ...
                    when (endringRequest) {
                        ...
                        is InnholdRequest -> {
                            oppdatertDeltaker.deltakelsesinnhold.shouldNotBeNull().innhold shouldBe
                                endringRequest.deltakelsesinnhold.innhold
        ...
        @Test
        fun `oppdaterDeltaker(deltakerOppdatering) - reaktivering med kladd - sletter kladd`() =
            runTest {
...
fun DeltakerEndring.Aarsak.toDeltakerStatusAarsak() =
