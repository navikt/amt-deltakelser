        ...
        scope.launch {
            if (leaderElection.isLeader() && attributes.getOrNull(isReadyKey) == true) {
                try {
                    ...
                    val tilgangerSomskalStenges = selfServiceTilgangService.getUtdaterteTiltakskoordinatorTilganger()
                    tilgangerSomskalStenges.forEach {
                        selfServiceTilgangService.stengTiltakskoordinatorTilgang(it.id)
                    }
