...
class JobManagerTest {
    @Test
    fun `startJob - to jobber - skal kjøre samtidig`(): Unit =
        runBlocking {
    ...
    @Test
    fun `stopJobs - skal stoppe jobber`() =
        runTest {
            val jobManager = JobManager(isLeader = { true }, applicationIsReady = { true })
            val counter = AtomicInteger(0)
    ...
    @Test
    fun `startJob - jobb med lang kjøretid - skal ikke starte nye jobber før forrige er ferdig`() =
        runTest {
            val jobManager = JobManager(isLeader = { true }, applicationIsReady = { true })
            val counter = AtomicInteger(0)
    ...
    @Test
    fun `startJob - skal respektere delay og period`() =
        runTest {
            val jobManager = JobManager(isLeader = { true }, applicationIsReady = { true })
